...
def load_ai_modules():
    ...
    각 역할 파일에는 과거 Gemini 직접 호출을 위한 import-time key check가
    남아 있을 수 있다. 실제 분석 호출은 ai_router가 담당하므로, Gemini 키가
    없어도 OpenRouter fallback까지 도달할 수 있도록 import만 느슨하게 한다.
    """
    modules = {}
    original_key = os.environ.get("GEMINI_API_KEY")

    if not original_key:
        # role 모듈의 import-time guard만 통과시키는 더미 값.
        # 실제 모델 호출은 ai.ai_router가 환경의 실제 키를 다시 읽어 처리한다.
...
def save_meeting_file(name, data):
    path = os.path.join(
        HISTORY_DIR,
        f"{name}_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}.json",
    )
    save_json(path, data)
    return path
...
def detect_intent(text, tickers):
    ...
    portfolio_keywords = [
        "내 계좌", "내계좌", "내 포트폴리오", "내포트폴리오", "계좌", "포트폴리오",
        "보유종목", "보유 종목", "전체 자산", "비중", "내 자산", "계좌 전체", "계좌상태", "계좌 상태",
    ]
    market_keywords = [
        "미국 증시", "미국시장", "미국 시장", "시장 분위기", "시장 상황", "매크로",
        "금리", "나스닥", "s&p", "vix", "연준", "fed",
    ]
    trading_keywords = [
        "사도", "살까", "매수", "매수해", "들어가", "진입", "팔까", "팔아", "팔아야",
        "매도", "얼마에 팔", "얼마에팔", "손절", "익절", "목표가",
    ]
    comparison_keywords = ["비교", "뭐가 나아", "뭐가 좋아", "둘 중", "어느 게", "어떤 게", "어느쪽"]

    if any(k in text_lower for k in portfolio_keywords):
        return "portfolio_stock" if tickers else "portfolio"
    if any(k in text_lower for k in market_keywords):
        return "market"
    if len(tickers) >= 2 or any(k in text_lower for k in comparison_keywords):
        return "comparison"
    if tickers:
        return "stock_decision" if any(k in text_lower for k in trading_keywords) else "stock_analysis"
    return "general"
...
내부적으로는 현재 계좌 원본, 독립 분석, 통합 토론을 교차검증한다.
그러나 사용자에게는 긴 연구보고서를 출력하지 않는다.

사용자가 실제로 질문한 대상은: {ticker_label}
위 대상 각각을 빠뜨리지 말고 별도로 판단한다.

행동은 다음 중 근거에 맞는 것을 선택한다:
적극 매수 / 조건부 매수 / 추가매수 / 보유 / 관망 / 일부매도 / 전량매도 / 비중조절

손절·익절·매도가를 요청했다면 고정 +10%, +20% 규칙을 사용하지 않는다.
실제 변동성, ATR, 지지/저항, 추세, 기업 상황, 실적, 뉴스, 시장환경,
평균매수가와 계좌 비중을 종합한다.
...
"""
        ...
        team_for_cat = {
            key: {"file": None, "content": normalize_result(value)}
            for key, value in team_results.items()
        }

        try:
            cat_result = cat.analyze(
                question=parsed["question"] + "\n\n" + concise_instruction,
                ticker=ticker_label,
                team_analyses=team_for_cat,
