...
def load_ai_modules():
    ...
    for name in ["crow","snake","raccoon","turtle","cat"]:
        try: modules[name] = importlib.import_module(f"ai.{name}")
            ...
            print(f"⚠️ {name}.py 불러오기 실패: {e}"); modules[name] = None
    return modules
...
def save_json(path, data):
    try:
        ...
        print(f"⚠️ JSON 저장 실패: {e}"); return False
...
def extract_tickers(text):
    ...
    found += [t for a,t in TICKER_ALIASES.items() if a.lower() in lower]
...
def get_account_data():
    ...
    try:
        ...
        if not data: raise RuntimeError("토스 API에서 계좌정보가 비어 있습니다.")
...
def show_request(parsed):
    print("\n"+"="*70); print("🧠 질문 분석")
    print(f"사용자 질문 : {parsed['question']}"); print(f"분석 유형   : {parsed['intent']}")
    print("분석 종목   : "+(", ".join(parsed["tickers"]) if parsed["tickers"] else "전체 시장 / 포트폴리오")); print("="*70)


def save_meeting_file(name,data):
    path=os.path.join(HISTORY_DIR,f"{name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    save_json(path,data); return path


def run_meeting(parsed,modules,account_data):
    from ai.batch_engine import run_team_batches
    tickers=parsed.get("tickers",[])
    print("\n"+"="*70); print("⚔️ AI TRADING TEAM 회의"); print("="*70)
...
내부적으로는 원본 데이터와 4명의 의견을 충분히 교차검증한다. 그러나 사용자에게는 긴 연구보고서를 출력하지 않는다.
종목별로 먼저 명확한 행동을 제시한다: 적극 매수 / 조건부 매수 / 추가매수 / 보유 / 관망 / 일부매도 / 전량매도 / 비중조절 중 실제 근거에 맞는 것을 선택한다.
사용자가 손절·익절·매도가를 요청했다면 실제 변동성, ATR, 지지·저항, 추세, 기업 상황, 실적, 뉴스, 시장환경, 평균매수가와 계좌 비중을 종합하여 가격을 제시한다. +10%, +20% 같은 고정 비율은 임의로 사용하지 않는다.
가격 근거가 부족하면 숫자를 만들지 않는다.
최종 답변은 가능한 한 다음 순서로 압축한다:
1. 🐱 알프레도 최종 판단
2. 종목별 행동 + 현재가
3. 종목별 핵심 기준 가격(가능한 경우)
4. 가장 중요한 이유 2~4개
5. 🐦🐍🦝🐢 각자의 핵심 의견을 1줄씩
긴 교차검증 표와 반복 설명은 생략한다.
"""
        cat_question=parsed["question"]+"\n\n"+concise_instruction
        team_for_cat={k:{"file":None,"content":normalize_result(v)} for k,v in team_results.items()}
        try:
            cat_result=cat.analyze(question=cat_question,ticker=parsed.get("primary_ticker"),team_analyses=team_for_cat,account_context=account_data)
        except Exception as e:
    ...
    print("\n"+"━"*70); print("🐱 알프레도 최종 판단"); print("━"*70)
    print(normalize_result(cat_result) if cat_result else "❌ 최종 판단을 생성하지 못했습니다.")
    print("━"*70); print(f"💾 최종 회의록: {final_path}")
    return final_data


def main():
    print("\n"+"="*70); print("🏦 AI TRADING TEAM / 돈물원"); print("⚡ 빠른 통합 투자 분석 시스템"); print("="*70)
    ...
    while True:
        ...
        parsed=parse_question(question); show_request(parsed)
        account_data=get_account_data(); save_json(AI_PORTFOLIO_FILE,account_data)
