...
class PixelAgent:
    # Each agent is drawn on a fine pixel grid, with a larger sprite footprint.
    DATA={
    ...
    def paint(self,p,x,y,scale=4):
        # 3 px logical pixels, enlarged sprite; deliberately blocky but detailed.
        ...
        p.setBrush(c1); p.drawRect(ox+15*scale,oy+8*scale,35*scale,27*scale)
        p.drawRect(ox+21*scale,oy+3*scale,22*scale,7*scale)
        if typ=='turtle':
            p.setBrush(c1); p.drawRect(ox+8*scale,oy+25*scale,49*scale,20*scale); p.setBrush(c2); p.drawRect(ox+18*scale,oy+28*scale,29*scale,13*scale)
            p.setBrush(c3); p.drawRect(ox+28*scale,oy+31*scale,9*scale,7*scale)
        elif typ=='bird':
            ...
            p.setBrush(c1); p.drawRect(ox+12*scale,oy+1*scale,9*scale,12*scale); p.drawRect(ox+44*scale,oy+1*scale,9*scale,12*scale)
            p.setBrush(c3); p.drawRect(ox+25*scale,oy+16*scale,6*scale,6*scale); p.drawRect(ox+39*scale,oy+16*scale,6*scale,6*scale)
        # face pixels
...
class PixelOffice(QWidget):
    ...
    def paintEvent(self,e):
        ...
        # long window, never occupied by door
        ...
        # door is immediately beside Hyeonmu, left of first desk, below window
        ...
        for n,a in self.agents.items():
            x,y,_=PixelAgent.DATA[n]; a.paint(p,x,y,4)
    ...
    def draw_city(self,p,x,y,w,h):
        ...
        for i in range(24):
            bw=28+(i%4)*13; bh=55+(i*17)%125; bx=x+8+i*int((w-16)/24); p.drawRect(bx,y+h-bh,bw,bh)
    ...
    def draw_weather(self,p):
        p.setPen(QColor('#a9c6d9')); p.setBrush(QColor('#a9c6d9'))
