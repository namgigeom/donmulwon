import os, sys, io, contextlib, traceback, re, html
...
class LiveStream:
    def __init__(self,emit): self.emit=emit; self.buf=''
    def write(self,text):
        if not text:return 0
        self.buf+=text
        while '\n' in self.buf:
            line,self.buf=self.buf.split('\n',1)
            if line.strip():self.emit(line.strip())
        return len(text)
    def flush(self):
        if self.buf.strip():self.emit(self.buf.strip());self.buf=''

class AnalysisWorker(QObject):
    output=Signal(str); progress=Signal(str); finished=Signal(); failed=Signal(str)
    def __init__(self,question):super().__init__();self.question=question
    @Slot()
    def run(self):
        live=LiveStream(self.progress.emit)
        try:
            import main
            self.progress.emit('⚡ 분석 엔진 시작 · 질문을 해석하고 있습니다...')
            with contextlib.redirect_stdout(live),contextlib.redirect_stderr(live): result=main.run_analysis(self.question)
            live.flush()
            answer=str(result.get('alfredo','') if isinstance(result,dict) else result or '').strip()
            if not answer:answer='알프레도의 최종 판단이 생성되지 않았습니다.'
            self.progress.emit('✅ 분석 완료 · 알프레도 최종 판단을 표시합니다.')
            self.output.emit(answer+'\n\n[MEETING_LOG]\n'+live.buf[-12000:])
        except Exception:self.failed.emit(traceback.format_exc())
        finally:self.finished.emit()

class DialoguePanel(QTextBrowser):
    COLORS={'현무':'#668a62','김선달':'#666b78','이묵':'#829b5e','너부리':'#8d8178','알프레도':'#c09a5b'}
    ...
    def set_meeting(self,text):
        out='<div style="font-family:Malgun Gothic;color:#eee4d2;font-size:10pt">';count=0
        for n in ['현무','김선달','이묵','너부리']:
            b=self._section(text,n)
            if b:out+=f'<div style="margin:4px;padding:8px;border:1px solid {self.COLORS[n]};background:#202427;border-radius:7px"><b>{self.ICONS[n]} {n}</b><br>{html.escape(b)}</div>';count+=1
        m=re.search(r'###\s*🐱\s*알프레도.*?(?=###\s*|\Z)',text,re.S)
        if m:
            b=re.sub(r'#+|[*`]','',m.group(0)).strip()[:900]
            out+=f'<div style="margin:7px 4px;padding:10px;border:2px solid #b99a63;background:#29231b;border-radius:7px"><b>🐱 알프레도 · FINAL VERDICT</b><br>{html.escape(b).replace(chr(10),"<br>")}</div>';count+=1
        if not count:out+='<div style="padding:18px;color:#9f9687">회의 결과가 아직 없습니다.</div>'
        self.setHtml(out+'</div>')
...
class MainWindow(QMainWindow):
    ...
    def update_clock(self):
        ...
        if self.thread is not None and self.analysis_started is not None:self.status.setText(f'⚔️ AI TRADING TEAM 분석 중 · {int((datetime.now()-self.analysis_started).total_seconds())}초 경과 · 4인 분석 → 토론 → 알프레도 검증')
    def start_analysis(self):
        q=self.input.text().strip()
        if not q or self.thread is not None:return
        self.analysis_started=datetime.now();self.office.characters.summon_for_question(overtime=True);self.office.update();self.button.setEnabled(False);self.input.setEnabled(False);self.progress.show();self.progress.setValue(5);self.dialogue.setHtml('<div style="color:#bca87e;padding:20px;">⚔ AI TRADING TEAM 긴급 호출<br><br>출입문 → 회의실 이동 중...<br>퇴근 시간에도 긴급 호출을 받으면 즉시 출근합니다.</div>');self.status.setText('🚨 긴급 호출 · 팀원 전원 출입문에서 회의실로 이동 중...')
        self.thread=QThread(self);self.worker=AnalysisWorker(q);self.worker.moveToThread(self.thread);self.thread.started.connect(self.worker.run);self.worker.progress.connect(self.on_progress);self.worker.output.connect(self.on_output);self.worker.failed.connect(self.on_failed);self.worker.finished.connect(self.thread.quit);self.worker.finished.connect(self.worker.deleteLater);self.thread.finished.connect(self.thread.deleteLater);self.thread.finished.connect(self.analysis_done);self.thread.start()
    def on_progress(self,line):
        pct=self.progress.value()
        if '토스증권' in line or '계좌정보' in line:pct=max(pct,20)
        elif '4인 독립 분석' in line or '분석 완료' in line:pct=max(pct,55)
        elif '통합 토론' in line:pct=max(pct,70)
        elif '알프레도' in line:pct=max(pct,85)
        else:pct=min(90,pct+2)
        self.progress.setValue(pct);self.status.setText('⚔️ '+line)
    def on_output(self,text):self.raw.setPlainText(text);self.dialogue.set_meeting(text);self.progress.setValue(100);self.status.setText('✅ 분석 완료 · 최종 판단 표시 완료')
    def on_failed(self,err):self.raw.setPlainText(err);self.dialogue.setHtml('<div style="color:#e29a8d;padding:18px;"><b>❌ 분석 실패</b><br><br>'+html.escape(err).replace(chr(10),'<br>')+'</div>');self.progress.setValue(0);self.status.setText('❌ 분석 실패 · 오류 로그를 확인하세요')
    def analysis_done(self):
        self.thread=None;self.worker=None;self.analysis_started=None;self.button.setEnabled(True);self.input.setEnabled(True);self.progress.hide()
        if not self.status.text().startswith('❌'):self.status.setText('분석 완료 · 현재 시각 기준 사무실 상태 유지')
