...
class PixelOffice(QWidget):
    ...
    def paintEvent(self, event):
        ...
        try:
            ...
            for name, (x, y, _) in self.characters.POSITIONS.items():
                p.drawText(QRect(int(x-35), int(y+70), 140, 22), Qt.AlignCenter, str(name))
    ...
    def draw_desks(self, p, w):
        ...
        for x, y in stations:
            ...
            p.setBrush(QColor('#705039')); p.drawRect(x+6, y+6, 163, 58)
        ...
        p.setBrush(QColor('#20272c')); p.drawRect(x+40, y+11, 140, 43)
        p.setBrush(QColor('#3c4649')); p.drawRect(x+49, y+18, 122, 30)
    ...
    def draw_conference_area(self, p, w, h):
        ...
        for i in range(1,6):
            xx = chart_x+12 + i*(chart_w-24)//6; p.drawLine(xx, grid_top, xx, grid_bottom)
        values = [0.48,0.43,0.52,0.49,0.58,0.55,0.66,0.61,0.72,0.68,0.79,0.75,0.86]
        points=[]; plot_w=chart_w-40; plot_h=grid_bottom-grid_top-6
        for i,v in enumerate(values):
            px=chart_x+20+int(i*plot_w/(len(values)-1)); py=grid_bottom-int(v*plot_h); points.append((px,py))
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle('돈물원 · DONMULWON'); self.resize(1500,900)
