...
class PixelCharacter:
    ...
    def paint(self,p,x,y,scale=2):
        ...
        R(8,36,24,2,'#111216');R(5,38,30,1,'#242326');R(9,22,22,12,c['suit']);R(7,25,4,8,c['dark']);R(29,25,4,8,c['dark']);R(12,23,16,10,'#303239');R(15,24,10,9,c['suit']);R(18,24,4,10,c['tie']);R(19,27,2,7,'#101116');R(10,33,7,4,'#101216');R(23,33,7,4,'#101216')
...
class CharacterLayer(BaseCharacterLayer):
    ...
    def __init__(self):
        super().__init__();self.characters={n:PixelCharacter(n) for n in self.POSITIONS};self._last_gui_state=None;self._state_file=os.path.join(os.path.dirname(os.path.abspath(__file__)),'meeting_state.json')
    ...
    def _apply_gui_state(self):
        ...
        elif stage in ('meeting','verdict'):self.mode='meeting'
        elif stage=='return':self.finish_meeting()
    def set_office_hours(self,hour,immediate=False):
        if immediate and self.mode in ('meeting_arrival','meeting','verdict'):return
        return super().set_office_hours(hour,immediate=immediate)
    def start_meeting(self):
        self.mode='meeting_arrival';self.move_speed=24;self.sequence=list(self.POSITIONS);self.index=0;self.ticks=0;self.queue=list(self.POSITIONS)
    ...
    def finish_meeting(self):
        ...
        if 9.0<=hour<18.0:self.reset_office()
        else:
    ...
    def tick(self):
        ...
        if self.mode=='meeting_arrival':
            ...
            for n in list(self.queue):
                ...
                if d<=self.move_speed:self.pos[n]=(float(tx),float(ty));self.queue.remove(n);arrived+=1
                else:self.pos[n]=(x+dx/d*self.move_speed,y+dy/d*self.move_speed)
            if not self.queue:self.mode='meeting'
            self.ticks+=1;return
        for n in list(self.queue):
            if self.mode=='leaving':tx,ty=self.DOOR
            else:tx,ty=self.POSITIONS[n][:2]
            x,y=self.pos[n];dx,dy=tx-x,ty-y;d=(dx*dx+dy*dy)**0.5;speed=18 if self.mode=='summoned' else self.move_speed
